...
echo_green "- Creating ${script_path}/multisig-policy.script from witness 1 and 2"
...
echo_green "- Creating ${key_path}/multisig-policy.addr"
...
echo_green "- Address string value : $(cat ${key_path}/multisig-policy.addr)"

read -p "Send ADA to script? (y/n) : " ans

if [[ ${ans} == "y" ]]; then
  echo_green "- Sending ADA to multisig-policy.addr"
