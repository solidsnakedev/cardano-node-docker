...
if [[ "$#" -eq 0 || "$#" -ne 5 ]]; then echo_red "Error: Missing parameters" && echo_yellow "Info: Command example -> update-asset.sh <wallet-name> <token-name> <amount-to-burn> <policy-name> <slot-number> "; exit 1; fi
...
echo_yellow "Info: Please run gen-policy-asset.sh or gen-policy-nft.sh ${policy_name}\n"; exit 1; fi
