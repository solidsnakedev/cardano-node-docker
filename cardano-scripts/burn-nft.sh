...
if [[ "$#" -eq 0 || "$#" -ne 5 ]]; then echo_red "Error: Missing parameters" && echo_yellow "Info: Command example -> burn-asset.sh <wallet-name> <token-name> <amount-to-burn> <policy-name> <slot-number> "; exit 1; fi
